...
from apps.learning.models import (
    Assignment, Lesson, LessonProgress, LessonWord, ReviewLog, UserStreak,
)
...
@pytest.fixture
def student(db):   return _user("student",  User.Role.USER,    db)
@pytest.fixture
def teacher(db):   return _user("teacher",  User.Role.TEACHER, db)
@pytest.fixture
def admin(db):     return _user("admin_u",  User.Role.ADMIN,   db)
...
@pytest.fixture
def ac(admin):   return _client(admin)
...
def lesson(db, teacher, word_a, word_b):
    ls = Lesson.objects.create(
        title="Lesson 1", level="A1", order_index=1,
...
def review_log(db, student, word_a):
    """ReviewLog với next_review_date = hôm nay (đến hạn)."""
